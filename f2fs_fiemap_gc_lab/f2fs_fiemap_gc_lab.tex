...
\item A device-side FIEMAP command if you want exact-file proof without a helper.
\end{itemize}

You do not need to build Android code for the basic workflow.  The no-build path
uses \texttt{adb}, root shell commands, F2FS sysfs knobs, donor files, and
\texttt{f2fs\_io fiemap} if the device already has it.  If \texttt{f2fs\_io}
is unavailable but \texttt{filefrag} exists, the harness falls back to
\texttt{filefrag}.  If the device has neither, the harness can still show
package paths, hashes, inodes, and F2FS GC counters, but it cannot prove that a
specific \texttt{base.apk}, ODEX, or VDEX file changed FIEMAP.

\section{No-Build Probe}
...
logs/device_tools.txt
\end{verbatim}

Interpretation:

\begin{itemize}[nosep]
\item \texttt{FIEMAP source: f2fs\_io}: exact-file FIEMAP comparison will use
      on-device \texttt{f2fs\_io fiemap}.
\item \texttt{FIEMAP source: filefrag}: exact-file FIEMAP comparison is possible
      through fallback \texttt{filefrag}.
\item \texttt{FIEMAP source: none}: the run can still create GC pressure and read
      F2FS counters, but per-file FIEMAP movement will be reported as
      \texttt{map-unk}.
\item \texttt{sysfs: not found}: the device kernel does not expose the expected
      F2FS sysfs controls to this root shell.
\end{itemize}

\section{Optional Native Helper}

The native helper is optional.  Use it only if you later have a way to run a
small device binary.  It adds direct \texttt{FS\_IOC\_FIEMAP}, ordinary F2FS GC
ioctl, and F2FS GC range ioctl support.
...
\begin{verbatim}
--helper ./f2fs_gc_probe.android --range-gc --gc-one-trials 128
\end{verbatim}
...
\item SHA-256;
\item FIEMAP extents through the helper, \texttt{f2fs\_io fiemap}, or fallback \texttt{filefrag};
...
cd tools/f2fs_fiemap_gc_lab
python3 f2fs_fiemap_gc_lab.py --probe-only
cat f2fs-gc-lab-*/probe_summary.txt
\end{verbatim}

Run an existing-install no-build test:
...
Run a controlled APK install no-build test:
...
classification, pinning, whether a FIEMAP source exists on the device, and
whether you can later use the optional GC range helper.
